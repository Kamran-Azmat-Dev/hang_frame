...
Rails.application.configure do
  ...
  config.paperclip_defaults = {
    storage: :s3,
    s3_protocol: 'https',
    s3_host_alias: Rails.application.credentials[:S3_PUBLIC_URL],
    url: ":s3_alias_url",
    path: 'public/:class/:attachment/:id/:style/:filename',
    s3_credentials: {
      bucket: Rails.application.credentials[:S3_BUCKET] || 'oregon',
  ...
  Rails.application.routes.default_url_options[:host] = ENV.fetch('DEFAULT_HOST', 'http://localhost:3000')
